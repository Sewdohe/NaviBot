...
function(ctx)
    ...
    navi.send_message(ctx.channel_id, {
        ...
        description = "Welcome to the server! To gain access to the rest of the channels, please prove you are human by clicking the button below.",
...
function(ctx)
    -- The DB auto-namespaces these to 'verification:verified_role', etc.
    local verified_role = navi.db.get("verified_role")
    local unverified_role = navi.db.get("unverified_role")
    local log_channel = navi.db.get("log_channel")
