...

    {
        ...
        if let Some(func) = callback {
            if let Ok(ctx_table) = lua.create_table() {
                ...
                let reply_embed_fn = {
                    ...
                    lua.create_function(move |_, (data, ephemeral): (mlua::Table, Option<bool>)| {
                        ...
                        tokio::spawn(async move {
                            let response = serenity::CreateInteractionResponse::Message(msg);
                            if let Err(e) = http.create_interaction_response(id, &token, &response, vec![]).await {
                                let _ = tx.send(crate::types::BotEvent::Log(LogLevel::Error, format!("reply_embed error: {}", e)));
                            }
                        });
                        Ok(())
                    })
                ...
                // ctx.defer(ephemeral?) — ACKs the interaction synchronously so
                // the plugin can do slow work then call ctx.followup().
                let defer_fn = {
                    let http = http.clone();
                    let token = interaction_token.clone();
                    let tx = tx_reply.clone();
                    lua.create_function(move |_, ephemeral: Option<bool>| {
                        let http = http.clone();
                        let token = token.clone();
                        let tx = tx.clone();
                        let result = tokio::task::block_in_place(|| {
                            tokio::runtime::Handle::current().block_on(async move {
                                let msg = serenity::CreateInteractionResponseMessage::new()
                                    .ephemeral(ephemeral.unwrap_or(false));
                                let response = serenity::CreateInteractionResponse::Defer(msg);
                                http.create_interaction_response(interaction_id, &token, &response, vec![]).await
                            })
                        });
                        if let Err(e) = result {
                            let _ = tx.send(crate::types::BotEvent::Log(LogLevel::Error, format!("defer error: {}", e)));
                        }
                        Ok(())
                    })
                };

                // ctx.followup(text, ephemeral?) — sends a follow-up after defer.
                let followup_fn = {
                    let http = http.clone();
                    let token = interaction_token.clone();
                    let tx = tx_reply.clone();
                    lua.create_function(move |_, (msg, ephemeral): (String, Option<bool>)| {
                        ...
                        tokio::spawn(async move {
                            let followup = serenity::CreateInteractionResponseFollowup::new()
                                .content(msg)
                                .ephemeral(ephemeral.unwrap_or(false));
                            if let Err(e) = http.create_followup_message(&token, &followup, vec![]).await {
                                let _ = tx.send(crate::types::BotEvent::Log(LogLevel::Error, format!("followup error: {}", e)));
                            }
                        });
                        Ok(())
                    })
                ...
                // ctx.followup_embed(data, ephemeral?) — embed follow-up after defer.
                let followup_embed_fn = {
                    let http = http.clone();
                    let token = interaction_token.clone();
                    let tx = tx_reply.clone();
                    lua.create_function(move |_, (data, ephemeral): (mlua::Table, Option<bool>)| {
                        ...
                        tokio::spawn(async move {
                            ...
                            let followup = serenity::CreateInteractionResponseFollowup::new()
                                ...
                                .ephemeral(ephemeral.unwrap_or(false));
                            if let Err(e) = http.create_followup_message(&token, &followup, vec![]).await {
                                let _ = tx.send(crate::types::BotEvent::Log(LogLevel::Error, format!("followup_embed error: {}", e)));
                            }
                        });
                        Ok(())
                    })
                ...
                if let Ok(reply) = reply_fn {
                    ...
                    if let Ok(d) = defer_fn {
                        let _ = ctx_table.set("defer", d);
                    }
                    if let Ok(f) = followup_fn {
                        let _ = ctx_table.set("followup", f);
                    }
                    if let Ok(fe) = followup_embed_fn {
                        let _ = ctx_table.set("followup_embed", fe);
                    }
